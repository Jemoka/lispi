...
impl From<MIRSegment> for EnrichedMIRSegment {
    fn from(seg: MIRSegment) -> Self {
        let blocks = seg.blocks.into_iter().map(|b| EnrichedMIRBasicBlock {
            statements: b.statements.into_iter()
                .map(|s| EnrichedMIRStatement { seg: s, dead: false })
                .collect(),
            // Preserve dead-block tagging from IR1's optimize → ir2 lower
            // — those blocks should stay dead post-MIR-SCCP too.
            state: if b.dead { BlockState::Bottom } else { BlockState::Top },
        }).collect();
        EnrichedMIRSegment {
...
impl From<EnrichedMIRSegment> for MIRSegment {
    fn from(e: EnrichedMIRSegment) -> Self {
        MIRSegment {
            next_reg: e.next_reg,
            blocks: e.blocks.into_iter().map(|b| MIRBasicBlock {
...
impl EnrichedMIRSegment {
    ...
    fn get_imm(&self, r: &ImmReg) -> SCCPState { self.get_id(r.0) }
    fn get_heap(&self, r: &HeapReg) -> SCCPState { self.get_id(r.0) }
        ...
        loop {
            if let Some(edge) = self.flow_wl.pop() {
                ...
                for i in 0..n {
                    ...
                    } else if matches!(stmt, MIRStatement::PhiOpImm(..) | MIRStatement::PhiOpHeap(..)) {
                        self.visit(edge.to, &stmt, &uses, &local_uses, &reach_out, &exec_edges);
                    } else {
                        break;
                    }
    ...
    fn visit(
        ...
        match s {
            ...
            CondBr { cond, then_blk, else_blk } => match self.get_imm(cond) {
    ...
    fn eval(&self, s: &MIRStatement) -> Option<(u32, SCCPState)> {
        ...
        if let Unbox(d, h) = s { return Some((d.0, self.get_heap(h))); }
        if let Box(src, dst) = s { return Some((dst.0, self.get_imm(src))); }
        if let Truthy(d, h) = s {
        ...
        match s {
            ...
            BinOr(d, a, b)  => Some((d.0, fold_bin(self.get_imm(a), self.get_imm(b), |x, y| x | y, |x, y| x | y))),
            BinAnd(d, a, b) => Some((d.0, fold_bin(self.get_imm(a), self.get_imm(b), |x, y| x & y, |x, y| x & y))),
            ...
            | Unbox(_, _) | Box(_, _) | Truthy(_, _) | UnboxLocal(_, _)
            | LoadLocal(_, _)
            | PushFrame | PopFrame
        ...
        for b in &mut self.blocks {
            for s in &mut b.statements {
                if let MIRStatement::BindLocal { name, id, src } = &s.seg {
                    if let SCCPState::Constant(v) = self.state.get(&src.0).cloned().unwrap_or(SCCPState::Top) {
                        s.seg = MIRStatement::BindImmediate {
                            name: name.clone(), id: *id, src: v,
                        };
                        continue;
                    }
                }
                if let MIRStatement::CondBr { cond, then_blk, else_blk } = &s.seg {
                    if let SCCPState::Constant(v) = self.state.get(&cond.0).cloned().unwrap_or(SCCPState::Top) {
                        let to = if is_falsy(&v) { *else_blk } else { *then_blk };
                ...
                if let Some((id, is_imm)) = dst_reg(&s.seg) {
                    if let SCCPState::Constant(v) = self.state.get(&id).cloned().unwrap_or(SCCPState::Top) {
    ...
    fn dead_blocks(&mut self) {
        ...
        while let Some(b) = wl.pop() {
            ...
            for s in &self.blocks[b].statements {
                ...
                match &s.seg {
                    ...
                    MIRStatement::CondBr { then_blk, else_blk, .. } => {
                        wl.push(*then_blk);
                        wl.push(*else_blk);
                    }
                    _ => {}
        ...
        for i in 0..n {
            self.blocks[i].state = if seen.contains(&i) { BlockState::Top } else { BlockState::Bottom };
        }
    }
}
...
fn has_side_effect(s: &MIRStatement) -> bool {
    use MIRStatement::*;
    !matches!(s,
        LoadValueImm(..) | LoadValuePtr(..)
        | LoadLocal(..) | LoadCapture(..)
        | Unbox(..) | Box(..) | Truthy(..) | UnboxLocal(..)
        | Add(..) | Sub(..) | Mul(..) | Div(..) | Mod(..) | Lshift(..) | Rshift(..)
        | BinNot(..) | BinOr(..) | BinAnd(..) | LogNot(..) | Xor(..)
        | Eq(..) | Gt(..) | Lt(..) | Gte(..) | Lte(..)
        | AsAddr(..) | AsSigned(..) | AsUnsigned(..)
        | Cons(..) | Car(..) | Cdr(..) | Nullp(..)
        | PhiOpImm(..) | PhiOpHeap(..) | Hits(..)
    )
}
...
fn dst_reg(s: &MIRStatement) -> Option<(u32, bool)> {
    ...
    match s {
        // imm dsts
        LoadValueImm(d, _)
        | Unbox(d, _) | Truthy(d, _) | UnboxLocal(d, _)
        ...
        | Car(d, _) | Cdr(d, _) | Array(d, _) | Unpack(d, _)
        | Escape(d, _)
            => Some((d.0, false)),
...
fn stmt_operands(s: &MIRStatement) -> Vec<u32> {
    ...
    match s {
        LoadValueImm(_, _) | LoadValuePtr(_, _)
        | LoadLocal(_, _) | LoadCapture(_, _) | UnboxLocal(_, _)
        ...
        StoreLocal(_, h) | StoreCapture(_, h) | Ret(h)
        | BindLocal { src: h, .. }
        | Car(_, h) | Cdr(_, h) | Nullp(_, h)
        | Array(_, h) | Unpack(_, h) | Hits(_, h)
        | Unbox(_, h) | Truthy(_, h)
        | Escape(_, h)
            => vec![h.0],

        BinNot(_, i) | LogNot(_, i)
        | AsAddr(_, i) | AsSigned(_, i) | AsUnsigned(_, i)
        | SysGet32(_, i) | SysUartPut8(_, i) | SysDelay(_, i)
            => vec![i.0],
...
fn substitute_imm_operands(s: &mut MIRStatement, alias: &BTreeMap<u32, u32>) {
    ...
    macro_rules! sub {
        ($x:expr) => {
            if let Some(&n) = alias.get(&$x.0) { $x.0 = n; }
        };
    }
    match s {
        // single imm operand
        ...
        SysZero32(_, a, b, c)    => { sub!(a); sub!(b); sub!(c); }
        SysFull32(_, a, b, c, d) => { sub!(a); sub!(b); sub!(c); sub!(d); }

        PhiOpImm(_, (_, a), (_, b)) => { sub!(a); sub!(b); }
