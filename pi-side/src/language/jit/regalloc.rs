...
use super::ir4::{Cond, Instr, Instruction, LIRBasicBlock, LIRSegment, Register, SpillSlot};
...
fn uses(p: &Pseudo) -> Vec<Operand> {
    ...
    match p {
        ...
        | Delay | ClearMonitor | GetMonitor | StopMonitor | Zero32 | StrMem | Full32 => vec![
            Operand::P(Register::R0),
            Operand::P(Register::R1),
            Operand::P(Register::R2),
            Operand::P(Register::R3),
        ],
...
fn walk_vregs(stmt: &RIRStatement, f: &mut impl FnMut(&VReg)) {
    ...
    match stmt {
        ...
        R::Unbox(d, s) | R::Box(d, s) | R::Truthy(d, s) => {
            f(d);
            f(s);
        }
        R::Add(d, a, b)
        ...
        R::Eq(d, a, b) | R::Gt(d, a, b) | R::Lt(d, a, b) | R::Gte(d, a, b) | R::Lte(d, a, b) => {
            f(d);
            f(a);
            f(b);
        }
        R::AsAddr(d, a) | R::AsSigned(d, a) | R::AsUnsigned(d, a) => {
            f(d);
            f(a);
        }
        R::Cons(d, a, b) => {
        ...
        R::Car(d, a) | R::Cdr(d, a) | R::Nullp(d, a) => {
            f(d);
            f(a);
        }
        R::Array(d, a) | R::Unpack(d, a) | R::Hits(d, a) => {
            f(d);
            f(a);
        }
        R::Full(d, a, b) | R::GetIdx(d, a, b) => {
            f(d);
            f(a);
            f(b);
        }
        ...
        R::ReadIdx(d, t, o, n) | R::FillIdx(d, t, o, n) => {
            f(d);
            f(t);
            f(o);
            f(n);
        ...
        R::Ret(s) => f(s),
        R::SysDsb(d)
        | R::SysPrefetchFlush(d)
        | R::SysUartInit(d)
        | R::SysUartGet8(d)
        | R::SysClearMonitor(d)
        | R::SysGetMonitor(d)
        | R::SysStopMonitor(d) => f(d),
        R::SysGet32(d, a) | R::SysUartPut8(d, a) | R::SysDelay(d, a) => {
            f(d);
            f(a);
        }
        R::SysPut32(d, a, b) => {
...
fn lower_stmt(stmt: &RIRStatement, out: &mut Vec<Pseudo>, next_call_cache: &mut u32) {
    ...
    match stmt {
        ...
        R::Eq(d, a, b) => {
            out.push(I::Cmp(v(a), v(b)));
            out.push(I::Cset(v(d), Cond::Eq));
        }
        R::Gt(d, a, b) => {
            out.push(I::Cmp(v(a), v(b)));
            out.push(I::Cset(v(d), Cond::Gt));
        }
        R::Lt(d, a, b) => {
            out.push(I::Cmp(v(a), v(b)));
            out.push(I::Cset(v(d), Cond::Lt));
        }
        R::Gte(d, a, b) => {
            out.push(I::Cmp(v(a), v(b)));
            out.push(I::Cset(v(d), Cond::Ge));
        }
        R::Lte(d, a, b) => {
            out.push(I::Cmp(v(a), v(b)));
            out.push(I::Cset(v(d), Cond::Le));
        }

        R::AsAddr(d, s) | R::AsSigned(d, s) | R::AsUnsigned(d, s) => out.push(I::Mov(v(d), v(s))),
        ...
        R::PhiOp(d, (pa, va), (pb, vb)) => out.push(I::Phi(v(d), (*pa, v(va)), (*pb, v(vb)))),
        R::Ret(s) => {
        ...
        R::SysDsb(d) => {
            out.push(I::Dsb);
            out.push(I::MovImm(v(d), ImmNumber::Integer(0)));
        }
        R::SysPrefetchFlush(d) => {
            out.push(I::PrefetchFlush);
            out.push(I::MovImm(v(d), ImmNumber::Integer(0)));
        }
        R::SysUartInit(d) => {
            out.push(I::UartInit);
        ...
        R::SysClearMonitor(d) => {
            out.push(I::ClearMonitor);
            out.push(I::MovImm(v(d), ImmNumber::Integer(0)));
        }
        R::SysGetMonitor(d) => {
            out.push(I::GetMonitor);
            out.push(I::Mov(v(d), p(Register::R0)));
        }
        R::SysStopMonitor(d) => {
            out.push(I::StopMonitor);
            out.push(I::MovImm(v(d), ImmNumber::Integer(0)));
        }
        ...
        R::Call { dst, callee, args } => {
            ...
            *next_call_cache += 1;
            ...
            out.push(I::MovImm(
                p(Register::R1),
                ImmNumber::Unsigned(args.len() as u32),
            ));
...
fn resolve_parallel_moves(moves: Vec<(Operand, Operand)>, next_vreg: &mut u32) -> Vec<Pseudo> {
    let mut moves: Vec<(Operand, Operand)> = moves.into_iter().filter(|(d, s)| d != s).collect();
...
fn liveness(blocks: &[Vec<Pseudo>], dead: &[bool]) -> BTreeMap<(usize, usize), BTreeSet<Operand>> {
    let n = blocks.len();
...
impl<'a> Coalescer<'a> {
    ...
    fn briggs_safe(&self, a: Operand, b: Operand) -> bool {
        let neighbors: BTreeSet<Operand> = self
            .graph
            .neighbors(&a)
    ...
    fn coalesce_pass(&mut self) -> bool {
        ...
        for (a, b) in moves {
            ...
            if !self.graph.nodes.contains(&a) || !self.graph.nodes.contains(&b) {
                continue;
            }
            if self.graph.edges.get(&a).map_or(false, |e| e.contains(&b)) {
                continue;
            }
            match (a, b) {
...
fn spill_rewrite(
    ...
    for blk in blocks.iter_mut() {
        ...
        for mut stmt in old {
            let used: Vec<Operand> = uses(&stmt)
                .into_iter()
                .filter(|o| spill_set.contains(o))
                .collect();
            let defd: Vec<Operand> = defs(&stmt)
                .into_iter()
                .filter(|o| spill_set.contains(o))
                .collect();
...
pub(crate) fn regalloc(seg: RIRSegment) -> LIRSegment {
    let (mut blocks, dead, mut next_vreg, call_args_max, call_cache_count) = lower_seg(seg);
